...
export default {
  ...
  methods: {
    ...mapActions("abc", ["getAllAccount", "cancelSearchAccounts", "delItemAccount", "takeData", "blockItemAccount", "getDataItemToEdit"]),
    styleSex,

    deleteItem(item) {
      confirm("Are you sure you want to delete this item?") && this.delItemAccount(item._id);
    },

    blockItem(item) {
      confirm("Are you sure you want to blocked this item?") && this.blockItemAccount({
        userId: item._id,
        blocked: true
      })
    },
